...
class UpLadderScreen(Screen):
    ...
    def load_steps_from_program_screen(self, preset_steps):
        self.clear_steps()
    ...
    def add_step(self, target_temp=20, target_time=30):
        ...
        step_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=100, spacing=30, padding=[20, 20, 0, 0])

        # Temp
        temp_box = BoxLayout(orientation='vertical', size_hint_x=None, width=100, spacing=5)
        temp_label = Label(text=str(step_data.target_temp), font_size=20, halign='center', valign='middle', text_size=(100, None))
        temp_box.add_widget(Label(text="Target °C", font_size=24, halign='left', valign='middle', text_size=(100, None)))
        ...
        temp_btns.add_widget(RoundedSmallButton(text="+1", size_hint=(None, None), size=(50, 50),
                                                on_press=start_increase_temp, on_release=lambda x: stop_increase_temp()))
        temp_btns.add_widget(RoundedSmallButton(text="-1", size_hint=(None, None), size=(50, 50),
                                                on_press=start_decrease_temp, on_release=lambda x: stop_decrease_temp()))
        ...
        time_label = Label(text=format_time(step_data.target_time), font_size=20, halign='center', valign='middle', text_size=(100, None))
        time_box.add_widget(Label(text="Time", font_size=24, halign='left', valign='middle', text_size=(100, None)))
        time_box.add_widget(time_label)
        step_data.bind(target_time=lambda instance, value: setattr(time_label, 'text', format_time(value)))

        def increase_time(dt=None):
        ...
        time_btns.add_widget(RoundedSmallButton(text="+1", size_hint=(None, None), size=(50, 50),
                                                on_press=start_increase_time, on_release=lambda x: stop_increase_time()))
        time_btns.add_widget(RoundedSmallButton(text="-1", size_hint=(None, None), size=(50, 50),
                                                on_press=start_decrease_time, on_release=lambda x: stop_decrease_time()))
    ...
    def open_panel(self):
        ...
        new_x = 0 if panel.x < 0 else -panel.width
